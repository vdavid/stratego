...
---------------------------------------------------------- */
var memoryGame = true; /* If true, then forgets discovered fields */
var serverSideScriptUrl = 'http://david.veszelovszki.hu/stratego-serverside.php';
var lastServerSideMove = '';
var colors = ['blue', 'red'];
...
function getServerSideMove() {
	return $.get(serverSideScriptUrl, null, serverSideMoveReceived);
}

function serverSideMoveReceived(data, textStatus, jqXHR) {
	if (data != lastServerSideMove) {
		if (data && (data != 'ready')) { console.log(data); } //autoMove(data); }
		lastServerSideMove = data;
	}
	setTimeout('getServerSideMove()', 2000);
}

function autoMove(move) {
	var fromField = getFieldByCoordinates(move.fromX, move.fromX);
	if (move.rank) { $(fromField).addClass(ranks[move.rank].shortName); }
...
//		getServerSideMove();
	...
	$('.stack' + /*'.' + currentPlayerColor	+*/ ' td').click(function(e) {
		if (currentGameStage != 'setup') { return; }
...
function boardFieldClickHandlerAtSetupTime(e) {
	...
	var selectedRank = '';
	if (selectedInStack.length) { selectedRank = getRank(selectedInStack); }
...
function boardFieldClickHandlerAtGameTime(e) {
	if ($(e.target).hasClass('possibleMove')) {
		...
		var movementStackItem = { fromX: getFieldX(selectedField), fromY: getFieldY(selectedField), fromColor: color, fromRank: rank,
		...
		if (moveType == 'move') {
			...
			if (isMovedPieceRevealed) { $(e.target).addClass('revealed'); }
...
function fight(attackerRank, defenderRank) {
	...
	if (attackerRank == defenderRank) { return 'tie'; } /* Two equal pieces tie */
	else if ((attackerRank == 'miner') && defenderRank == 'bomb') { return 'attacker'; } /* Miner kills bomb */
	else if ((attackerRank == 'spy') && defenderRank == 'marshal') { return 'attacker'; } /* Spy kills marshal */
	else if ((defenderRank == 'flag') || (defenderRank == 'spy')) { return 'attacker'; } /* Any piece kills a flag or a spy */
	else if ((Number(attackerRankShortName) == attackerRankShortName) && ((Number(defenderRankShortName) == defenderRankShortName))) {
		return attackerRankShortName < defenderRankShortName ? 'attacker' : 'defender'; /* A smaller number defeats a bigger one */
	} else { return 'defender'; } /* In any other case, defender wins */
}
...
function showPossibleMoves(tdElement) {
	...
	if ((rank == 'bomb') || (rank == 'flag')) {
		...
		for (i = 0; i < 4; i++) {
			...
			distance = 1;
			while(true) {
				var adjacentField = getAdjacentField(lastField, directions[i]);
				...
				if ((getColor(adjacentField) == color) || $(adjacentField).hasClass('water')) {
					...
					if (distance > 1) { $(adjacentField).addClass('scoutMove'); }
		...
		for (i = 0; i < 4; i++) {
			var adjacentField = getAdjacentField(tdElement, directions[i]);
...
function getAdjacentField(tdElement, direction) {
	if (direction == 'up') { return getFieldByCoordinates(getFieldX(tdElement), getFieldY(tdElement) - 1); }
	else if (direction == 'right') { return getFieldByCoordinates(getFieldX(tdElement) + 1, getFieldY(tdElement)); }
	else if (direction == 'down') { return getFieldByCoordinates(getFieldX(tdElement), getFieldY(tdElement) + 1); }
	else if (direction == 'left') { return getFieldByCoordinates(getFieldX(tdElement) - 1, getFieldY(tdElement)); }
}
...
function getRank(tdElement) {
	...
	for (var i = 0; i < classes.length; i++) {
		if (rankNames.indexOf(classes[i]) > -1) { return classes[i]; }
	}
...
function getColor(tdElement) {
	if ($(tdElement).hasClass('blue')) { return 'blue'; }
	else if ($(tdElement).hasClass('red')) { return 'red'; }
...
function selectNextInStack(tdElement) {
	...
	if (!lastElement || !lastElement.length) { return; }
	while(true) {
